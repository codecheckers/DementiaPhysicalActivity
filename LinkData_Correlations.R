...
Link19_21 <- read_csv("DataMedloADL/Link19_21.csv")
keydata<-as.data.frame(Link19_21)[,1:2]

AveragesPerResident[,4]<- as.numeric(AveragesPerResident[,4])

SensorData_21<- merge(AveragesPerResident, keydata, by.x = "residents", by.y = "LIACS ID" )

# add 24hrs data
#add colnames
Dat21_24hrs<-cbind(t(Normalized24hrsTo30min21), residents= as.numeric(colnames(Normalized24hrsTo30min21)))
...
MovementData_21 <- merge(WatchData_21, NormalizedTo30min, by.x = "NIVEL ID (Compas 2019)", by.y = "bwcode")
